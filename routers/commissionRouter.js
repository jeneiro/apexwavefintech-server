...
router.post("/update/:id", auth, async (req, res)=>{
    try {
       ...
       var newStatus = { $set: {withdrawalStatus: "Withdrawn" } };
        var myQuery = {_id:id}
       const updateStatus = await Commision.updateOne(myQuery, newStatus, function(err, res) {
    if (err) throw err;
...
router.get("/:id", auth,async (req,res)=>{
    try {
        const { id} = req.params;
       const commissions = await Commision.find({referrer:id, withdrawalStatus:"Balance"});
     ...
     const totalCommission = commissions.map(com=>{
      ...
      if (com.withdrawalStatus === 'Balance' ){
